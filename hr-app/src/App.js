...
function App() {
  ...
  useEffect(() => {
    const verifyToken = async () => {
      ...
      try {
        ...
        console.log('Estado inicial tras verificar token:', { employeeId: decodedToken.employee_id, userId: decodedToken.user_id, role, roles: decodedToken.roles });
      } catch (error) {
  ...
  }, [recognitionPage, fetchRecognitions]);
  ...
  const handleDelete = async (type, id) => {
    try {
      ...
      if (type === 'employees') {
        ...
        // Actualizar el dashboard si es admin
        if (userRole === 'admin') {
  ...
  const handleRestore = async (id) => {
    try {
      ...
      await fetchEmployees(token);
      if (userRole === 'admin') {
        await fetchDashboardData(); // Actualizar dashboard al restaurar
      }
      ...
      <ChangePassword
        ...
        setUser={(updatedUser) => {
          ...
          if (updatedUser.token) {
            ...
            console.log('Estado tras cambio de contraseña:', { employeeId: newDecodedToken.employee_id, userId: newDecodedToken.user_id, role: newDecodedToken.roles[0], roles: newDecodedToken.roles });
          }
